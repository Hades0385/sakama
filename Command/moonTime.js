...
module.exports = {
    ...
    run: async (message, args) => {
      ...
      axios
        .get(`https://opendata.cwa.gov.tw/api/v1/rest/datastore/A-B0063-001?Authorization=${CWA_API}&limit=1&format=JSON&CountyName=${args}&Date=${todayData}`)
        .then(response => {
            ...
            let moons = apiData.MoonSetTime;
            let country = apiInfo.CountyName
            const nowembed = new EmbedBuilder()
              .setColor('#000000')
              .setAuthor({ name: '月出月落時間', iconURL: 'https://pic.616pic.com/ys_bnew_img/00/24/17/0joFE1ygYw.jpg'})
              .setTitle(`${country}的月出月落時間`)
              .setDescription(`${data}`)
              .addFields(
                { name: '月出時間', value: `${moonr} `, inline: true },
                { name: '月落時間', value: `${moons} `, inline: true },
              )
              .setImage(`https://www.cwa.gov.tw/Data/astronomy/moon/${year}${month}${day}.jpg`)
              .setTimestamp()
